{{-- <section class="mt-16 reveal">
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="">
            ...
            <x-text-input id="name" name="name" type="text" class="mt-1 mb-4 block w-full" :value="old('name', $user->name)"
        ...
        <div>
            ...
            <x-text-input id="email" name="email" type="email" class="mt-1 mb-4 block w-full" :value="old('email', $user->email)"
                required autocomplete="username" />
            <x-input-error class="mt-2" :messages="$errors->get('email')" />

            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && !$user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-black dark:text-gray-200">
