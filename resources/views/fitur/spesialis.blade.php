        ...
        <div class="bg-gray-100 rounded-2xl shadow-lg w-full max-w-4xl p-8">
            ...
            <form action="{{ route('spesialis') }}" method="GET" class="space-y-6">
                <div class="flex justify-start items-center w-full space-x-4">
                    <div class="flex-grow">
                        <input type="text" name="nama" id="nama" class="block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600" placeholder="Enter specialist name" value="{{ request('nama') }}" />
                    </div>
                ...
                <div class="flex justify-start items-center w-full space-x-4">
                    ...
                    <input type="number" id="min_price" name="min_price" class="rounded-md border-0 py-1.5 px-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600" placeholder="Min" value="{{ request('min_price') }}" />
                    <span class="text-gray-500">-</span>
                    <input type="number" id="max_price" name="max_price" class="rounded-md border-0 py-1.5 px-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600" placeholder="Max" value="{{ request('max_price') }}" />
                </div>
            </form>
        </div>
    </div>
            ...
            <form id="locationForm" action="{{ route('spesialis') }}" method="GET">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex items-center">
                        <input type="radio" id="di-yogyakarta" name="location" value="D.I Yogyakarta" class="mr-2" onchange="this.form.submit()" {{ request('location') == 'D.I Yogyakarta' ? 'checked' : '' }}>
                        <label for="di-yogyakarta">D.I Yogyakarta</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="sleman" name="location" value="Sleman" class="mr-2" onchange="this.form.submit()" {{ request('location') == 'Sleman' ? 'checked' : '' }}>
                        <label for="sleman">Sleman</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="bantul" name="location" value="Bantul" class="mr-2" onchange="this.form.submit()" {{ request('location') == 'Bantul' ? 'checked' : '' }}>
                        <label for="bantul">Bantul</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="kulon-progo" name="location" value="Kulon Progo" class="mr-2" onchange="this.form.submit()" {{ request('location') == 'Kulon Progo' ? 'checked' : '' }}>
                        <label for="kulon-progo">Kulon Progo</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" id="Gunung-Kidul" name="location" value="Gunung Kidul" class="mr-2" onchange="this.form.submit()" {{ request('location') == 'Gunung Kidul' ? 'checked' : '' }}>
